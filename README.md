...
This project provides a Docker container for [Avidemux](https://avidemux.org).
...
container, it would proxy all HTTP requests for `avidemux.domain.tld` to
...
server {
	...
	location /avidemux/ {
		...
		# Needed when audio support is enabled.
		location /avidemux/websockify-audio {
			proxy_pass http://docker-avidemux/websockify-audio;
			proxy_http_version 1.1;
			proxy_set_header Upgrade $http_upgrade;
			proxy_set_header Connection $connection_upgrade;
			proxy_read_timeout 86400;
...
| **Application version** label | Displays the version of Avidemux integrated into Docker image. |
...
Any desktop notification generated by Avidemux is forwarded to the
